...
map('n', '<LEADER>c', ':let @+ = fnamemodify(expand("%"), ":.") . ":L" . line(".")<CR>', {}) --c: 現在のファイル名と行番号をクリップボードにコピー (copy)
map('v', '<LEADER>c', ':<C-u>let @+ = fnamemodify(expand("%"), ":.") . ":L" . line("\'<") . "-" . line("\'>") <CR>', {}) --c: [Visual] 選択範囲のファイル名と行番号をクリップボードにコピー (copy)
